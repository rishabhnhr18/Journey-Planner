...
5.  SOLVER-ONLY OUTPUT — no post-solve modifications.
    The solver uses circuit-based routing with real pairwise distances.
    What the solver returns is the final route — no post-solve modifications.
...
def validate_schedule(
    ...
    # Circuit constraint enforces this budget inside the solver.
    # ─────────────────────────────────────────────────────────────────────────
    time_viol = []
    for (sid, date), grp in grouped:
    ...
    if ("estimated_travel_minutes" in detailed.columns
        ...
        for (sid, date), grp in grouped:
            ...
            if abs(stored - actual_avg_leg_min) > 5:   # ±5 min tolerance
    ...
    if "route_leg_km" in detailed.columns and "route_rank" in detailed.columns:
        for (sid, date), grp in detailed.groupby(["sales_id", "schedule_date"]):
            ...
            for _, row in grp_sorted.iterrows():
                ...
                if abs(stored_leg - expected_leg) > 0.5:   # tolerance 500 m
                    leg_viol.append(
    ...
    checks = {
        ...
        "Travel minutes (avg inter-leg, ±5 min)":   travel_viol,
...
#             Solver output is final — no post-solve modifications.
#
