...
#             # ── Travel time: simulate NN route → separate wh leg from inter-customer legs
#             #
#             # OLD BUG: used warehouse_to_customer_km per stop → 4× over reality.
#             #
#             # FIX: run the same nearest-neighbour route that DailyRoutePlanner uses.
#             # This gives two values kept SEPARATE to avoid double-counting:
#             #   wh_leg_km    — warehouse → first customer (fixed daily cost)
#             #   avg_inter_km — average inter-customer leg (charged per stop)

#             cust_coords: list[tuple[float, float]] = [
#                 (float(customers.iloc[i]["gps_lat"]), float(customers.iloc[i]["gps_lng"]))
#                 for i in range(len(customers))
#             ]

#             if len(cust_coords) > 1:
#                 _unvisited = list(range(len(cust_coords)))
#                 _cur_lat, _cur_lng = warehouse_lat, warehouse_lng
#                 _route_legs: list[float] = []
#                 while _unvisited:
#                     _ni = min(
#                         _unvisited,
#                         key=lambda i: _haversine_km(
#                             _cur_lat, _cur_lng,
#                             cust_coords[i][0], cust_coords[i][1])
#                     )
#                     _leg = _haversine_km(
#                         _cur_lat, _cur_lng,
#                         cust_coords[_ni][0], cust_coords[_ni][1])
#                     _route_legs.append(_leg)
#                     _cur_lat, _cur_lng = cust_coords[_ni]
#                     _unvisited.remove(_ni)
#                 wh_leg_km    = _route_legs[0]
#                 _inter_legs  = _route_legs[1:]
#                 avg_inter_km = float(np.mean(_inter_legs)) if _inter_legs else wh_leg_km
#             else:
#                 wh_leg_km    = (
#                     _haversine_km(warehouse_lat, warehouse_lng,
#                                 cust_coords[0][0], cust_coords[0][1])
#                     if cust_coords else 5.0
#                 )
#                 avg_inter_km = wh_leg_km

#             # round() not int() — avoids systematic underestimation
#             wh_leg_min         = max(0, round(wh_leg_km    / avg_speed_kmph * 60))
#             avg_leg_travel_min = max(1, round(avg_inter_km / avg_speed_kmph * 60))

#             # Effective budget = full day minus fixed warehouse departure.
#             # Used in BOTH sp_daily_cap AND Constraint 6 — must be consistent.
            
#             #done done
#             # effective_daily_minutes = daily_work_minutes - wh_leg_min
#             effective_daily_minutes = daily_work_minutes

#             print(f"  [{territory_id}/{truck_group}] route estimate: "
#                 f"wh={wh_leg_km:.1f}km({wh_leg_min}min) "
#                 f"avg_inter={avg_inter_km:.2f}km({avg_leg_travel_min}min) "
#                 f"effective_budget={effective_daily_minutes}min")

#             travel_mins: dict[tuple[str, str], int] = {}
#             for _, cust in customers.iterrows():
#                 cid = cust["customer_id"]
#                 for sid in sp_ids:
#                     travel_mins[(cid, sid)] = avg_leg_travel_min

#             print("Trvel Minutes:",travel_mins)
            
#             # ── Per-SP daily capacity ─────────────────────────────────────
#             # travel_mins is uniform (avg_leg_travel_min). effective_daily_minutes
#             # already deducts the warehouse departure. sp_time_per_visit = visit +
#             # avg inter-leg only (warehouse leg is NOT charged per stop).
            
#             sp_time_per_visit: dict[str, int] = {
#                 sid: max(1, avg_visit_minutes + avg_leg_travel_min)
#                 for sid in sp_ids
#             }
#             sp_daily_cap: dict[str, int] = {
#                 sid: max(1, effective_daily_minutes // sp_time_per_visit[sid])
#                 for sid in sp_ids
#             }
#             print(f"  [{territory_id}/{truck_group}] sp_daily_cap="
#                 f"{list(sp_daily_cap.values())[0]} stops/day "
#                 f"({avg_visit_minutes}min visit + {avg_leg_travel_min}min inter "
#                 f"= {sp_time_per_visit[sp_ids[0]]}min/stop, "
#                 f"budget={effective_daily_minutes}min)")
...
#             # sum(visit_min + inter_leg_min per stop) ≤ effective_daily_minutes
#             # effective_daily_minutes = daily_work_minutes − wh_leg_min
#             # Warehouse departure already deducted; travel_mins = inter-leg only.
#             for sid in sp_ids:
#                 for d in valid_dates:
#                     model.Add(
#                         sum(
...
#                             # avg_leg_travel_min = inter-customer avg used in solver budget.
#                             # DailyRoutePlanner fills route_leg_km with exact legs post-solve.
#                             "estimated_travel_minutes": avg_leg_travel_min,
#                             "warehouse_to_customer_km": round(wh_dist_km, 3),
#                             "route_leg_km":             None,
...
#                     f"(capacity overflow — see unvisited_customers / under_visited_customers): {missed[:5]}"
...
#     def __init__(self, solver_time_seconds: int = 180):
#         self.solver_time_seconds = solver_time_seconds
#     def solve(
#             self,
#             customers: pd.DataFrame,           # pre-filtered for this territory + truck group
