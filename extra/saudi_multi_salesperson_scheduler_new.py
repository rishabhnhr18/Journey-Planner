...
class CustomerAssigner:
    ...
    def _rebalance_high_segment(customers: pd.DataFrame, sp_ids: list[str]) -> pd.DataFrame:
        """
        Spread High-segment customers evenly across salespeople.
        Within High segment, sort by final_customer_score descending so the
        best customers are distributed first.
        """
        ...
        # sort by final_customer_score if available, else by customer_id
...
class MultiSalespersonScheduler:
    ...
    def create_schedules(
        ...
        for _, ter_row in territory_df.iterrows():
            ...
            # For debugging
            ter_customers.to_excel("ter_customer.xlsx")
